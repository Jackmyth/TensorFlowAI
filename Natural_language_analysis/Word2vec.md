...
2.word2vec 計算的是餘弦值 (cosine)，距離範圍為 0–1 之間，值越大代表兩個詞關聯度越高。
詞向量表示法讓相關或者相似的詞，在維度距離上更為接近。

3.同詞的繁體字、簡體字用word2vec轉成向量，會形成兩個向量。這用Opencc套件將簡體字轉成繁體字
，就可以解決向量重複的問題。
...
這時將數字list做截長補短，讓數字list長度固定在100，
之後數字list轉向量list需固定長度，在做深度學習模組比較好訓練。
